...
#n, sample size
#p0=70 (fixed), number of nonzero beta
...
glm_wrapper=function(n, p0, p1, rho, threshold, iter, B, train.pcntg){
  ...
  stabel_iter=foreach(i = 1:iter, .packages = c("MASS","stabs", "sparseSVM", "Boruta", "made4", "SuperLearner", "pROC", "glmnet")) %dopar%{
    ...
    set.seed(2000+i)
    stab.rconcave=stabsel(x=df[train,-(p1+1)], y=df[train,p1+1],fitfun=glmnet.lasso, cutoff=threshold, PFER = 80, sampling.type = "SS", assumption="r-concave")
    ...
    for (j in 1:B){
      ...
      svm.mod1=sparseSVM(xx.svm1,yy.svm1, alpha=1, gamma=0.1,lambda=0.05, max.iter=1000, dfmax=p1+1) #dfmax=p+1
      ...
      svm.mod2=sparseSVM(xx.svm2,yy.svm2, alpha=1, gamma=0.1,lambda=0.05, max.iter=1000, dfmax=p1+1)
    ...
    for (k in 1:B){
      ...
      rf.mod1=Boruta(y ~ ., data = df.rf1, doTrace = 0, ntree = 200, pValue = 0.1)
      ...
      rf.mod2=Boruta(y ~ ., data = df.rf2, doTrace = 0, ntree = 200, pValue = 0.1)
...
result=glm_wrapper(n=600, p0=100, p1=800, rho=0.5, threshold=0.65, iter=100, B=50, train.pcntg=0.6)
