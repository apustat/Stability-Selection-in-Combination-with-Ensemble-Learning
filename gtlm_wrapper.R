...
#lam, tuning parameter for traditional LASSO

gtlm_wrapper=function(n, p0, p1, rho, threshold, iter, B, train.pcntg, lam){
  ...
  #signal_var=c(4.8,4.2,3.5,-4.1,-4.9)
  stabel_iter=foreach(i = 1:iter, .packages = c("stabs", "sparseSVM", "Boruta", "made4", "SuperLearner", "pROC", "glmnet","MASS")) %dopar%{
    set.seed(1000+i)
    ...
    b <- if (rho == 0) { #if rho=0, the true betas are fixed
      ...
      b_matrix[1:5] <- c(4.8,4.2,3.5,-4.1,-4.9)
      b_matrix
    ...
    error=rnorm(n,0,sqrt(4))
    y.cont=xx%*%b+error
    cutoff_y=1
    y=ifelse(y.cont<(median(y.cont)+cutoff_y),0,1)
    df=data.frame(x,y)
    ...
    ##############LASSSO
    set.seed(2000+i)
    stab.rconcave=stabsel(x=df[train,-(p1+1)], y=df[train,p1+1],fitfun=glmnet.lasso, cutoff=threshold, PFER = 3, sampling.type = "SS", assumption="r-concave")
    ...
    for (j in 1:B){
      ...
      svm.mod1=sparseSVM(xx.svm1,yy.svm1, alpha=1, gamma=0.6,lambda=0.10, max.iter=1000, dfmax=p1+1) #dfmax=p+1
      ...
      svm.mod2=sparseSVM(xx.svm2,yy.svm2, alpha=1, gamma=0.6,lambda=0.10, max.iter=1000, dfmax=p1+1)
    ...
    for (k in 1:B){
      ...
      sel.var.rf1 = which(rf.mod1$finalDecision == "Confirmed")
      
      df.rf2=df.train[-train.rf,]
      rf.mod2=Boruta(y ~ ., data = df.rf2, doTrace = 0, ntree = 300, pValue = 0.01)
      sel.var.rf2 = which(rf.mod2$finalDecision == "Confirmed")
    ...
    best_model <- glmnet(as.matrix(df.train[, -(p1+1)]), as.factor(df.train[,(p1+1)]),family="binomial", alpha=1, lambda = lam)
    final.set.lasso=which(coef(best_model)[-1] !=0)
...
result=gtlm_wrapper(n=600, p0=5, p1=200, rho=0, threshold=0.65, iter=10, B=50, train.pcntg=0.6, lam=0.055)
...
##traditional lasso, barely selects any variable, that's why tuning parameter is modified a little bit
##such that at least one variable is selected
# rho=0(independent), n=600, p=200, lam=0.055
# rho=0(independent), n=600, p=800, lam=0.067
# rho=0(independent), n=1000, p=500, lam=0.053
# rho=0(independent), n=1000, p=1500, lam=0.053
# 
# rho=0.5(correlated), n=600, p=200, lam=0.058
# rho=0.5(correlated), n=600, p=800, lam=0.071
# rho=0.5(correlated), n=1000, p=500, lam=0.051
# rho=0.5(correlated), n=1000, p=1500, lam=0.057
