...
glm_wrapper=function(n, p0, p1, rho, threshold, iter, B, train.pcntg){
  ...
  stabel_iter=foreach(i = 1:iter, .packages = c("MASS","stabs", "sparseSVM", "Boruta", "made4", "SuperLearner", "pROC", "glmnet")) %dopar%{
    ...
    for (j in 1:B){
      ...
      svm.mod1=sparseSVM(xx.svm1,yy.svm1, alpha=1, gamma=0.6,lambda=0.11, max.iter=1000, dfmax=p1+1) #dfmax=p+1
      ...
      svm.mod2=sparseSVM(xx.svm2,yy.svm2, alpha=1, gamma=0.6,lambda=0.11, max.iter=1000, dfmax=p1+1)
    ...
    for (k in 1:B){
      ...
      rf.mod1=Boruta(y ~ ., data = df.rf1, doTrace = 0, ntree = 100, pValue = 0.01)
      ...
      rf.mod2=Boruta(y ~ ., data = df.rf2, doTrace = 0, ntree = 100, pValue = 0.01)
    ...
    #final.set.lasso=which(coef(cvfit, lambda=30)[-1] !=0)
    lasso.mod=glmnet(as.matrix(df.train[, -(p1+1)]), as.factor(df.train[,(p1+1)]),family="binomial",alpha=1, lambda=0.11)
    final.set.lasso=which(coef(lasso.mod)[-1] !=0)
...
result=glm_wrapper(n=600, p0=5, p1=800, rho=0.5, threshold=0.65, iter=100, B=50, train.pcntg=0.6)
